...
cart_prod <- cross_join(distinct(HuntEventsreduced), tibble(sender_ids)) %>%
  rename(HuntEventTime = t_, HuntEventX = X, HuntEventY = Y, Sender.ID = sender_ids)

StressEvents <- cart_prod %>%
  left_join(Movement, join_by(Sender.ID, closest(HuntEventTime >= t_))) %>%
  rename(x_before = x_, y_before = y_, t_before = t_) %>%
  left_join(Movement, join_by(Sender.ID, closest(HuntEventTime <= t_))) %>%
  rename(x_after = x_, y_after = y_, t_after = t_) %>%
  na.omit() %>%
  mutate(InterpolatedX = NA, InterpolatedY = NA, Speed = NA, Distance = NA)

n_rows <- nrow(StressEvents)
for (i in 1:n_rows) {
  cat(sprintf("Processing row %d of %d", i, n_rows), "\r")

  current_row <- StressEvents[i, ]

  missing_before_or_after <- anyNA(current_row[c("t_before", "t_after")])
  if (missing_before_or_after) next

  hunt_time <- current_row$HuntEventTime
  
  # Get coordinates and time difference
  x1 <- current_row$x_before
  y1 <- current_row$y_before
  time1 <- current_row$t_before
