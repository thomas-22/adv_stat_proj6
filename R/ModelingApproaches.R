...
tune_xgboost <- function(X_train, y_train, X_test, y_test, 
  ...
  while (!tuning_converge && iteration_count <= max_iterations) {
    ...
    for (i in 1:nrow(param_grid)) {
      ...
      cat(sprintf("Iteration %d of %d: Evaluating %d of %d parameter sets...", iteration_count, max_iterations, i, nrow(param_grid)), "\r")
...
cat("RMSE After Random Permutation (Test Predictions):", comparisons$mean_rmse_test_random, "\n")
cat("RMSE After Random Permutation (Full Predictions):", comparisons$mean_rmse_full_random, "\n")
cat("RMSE After Training on Permuted Labels:", comparisons$fm_trainedon_permute_data_test_rmse, "\n")
