...
Here are some models I've been playing with. Of course it would better to encapsulate them in re-usable functions. --Baisu.

```{r}
library(mgcv)
library(ggeffects)

data_closest_time <- FilterClosestTime(
  data_full,
...
# Wait, what is the time difference between defecation and sampling?
# (DefecTime == Collar_t_, SampleTime == Waypoint_t_)
FCMStress %>% filter(Sample.ID == "K178A")
# Any other crazy mistakes like this?
FCMStress %>%
  mutate(SampleDelayDays = difftime(SampleTime, DefecTime, units = "days")) %>%
  filter(SampleDelayDays > days(1))
# Probably the researchers got the date format wrong.
```
