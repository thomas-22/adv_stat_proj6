...
model.data.augmented <- model.data %>%
  ...
  pivot_longer(c(simple.model, random.intercept.model, random.slope.model),
               values_to = ".fit",
               names_to = "model")

ggplot(model.data.augmented, aes(color = model)) +
  stat_qq_line(aes(sample = ng_g), color = "black") + 
  stat_qq(aes(sample = .fit), alpha = .7, size = 1) + 
  theme_light() +
  labs(y = "FCM level", title = "QQ-Plots of different model fits")
```
