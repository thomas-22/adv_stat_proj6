...
```{=html}
<!-- Run `HuntEvents %>% na.omit() %>% nrow()` gives 519. Previously I wrote 532, but that included hunting events with missing location. 519 is the number of hunting events that we can use for distance computation. So I'd say we just report this number. -->
```

::::: columns
::: {.column width="40%"}
Contains location and time of $\geq$ 700 hunting events
...
```{=html}
<!-- Baisu: In two dimensions (time and space), there is no "curse of dimensionality." This is just the wrong term. -->
```
