...
The XGBoost model outperforms both GAMM models on the "highest score" and "nearest" dataset in predictive power but performs worse on the "closest in time" dataset (see @tbl-rmse). After reviewing the hyperparameters and comparisons between models trained on the actual data and the data with randomized labels we concluded that the XGBoost models likely did not learn noise and therefore are not overfitting.
