...
iv) Day of year \[1-365\][^2] (*Defecation Day*)
v)  Number of hunts a deer experiences before defecation (*Number of Other Hunts*)

[^2]: Not all values between 1 and 365 were observed. As shown in @fig-samples_daily_count, faecal samples were not collected during winter.
...
Having identified the most relevant hunting event, covariate i) can be calculated easily as the time difference between the hunting event and the defecation event. To obtain covariate ii) we have to take in account, that the movement data is recorded in 1-hour intervals, while the hunting events occur at arbitrary time points. Therefore we need to approximate the exact deer positions at hunting event times. We interpolate deer positions at the timestamp of each hunting event, assuming constant velocity and linear movement.

Suppose a hunting event occurred at time $t$, but we only know the deer's location at time $t_0, t_1$ with $t_0 < t < t_1$. Then we approximate the location $r(t)$ of the deer at time $t$ by
\begin{equation*}
...
The *Time Difference* and *Distance* are of main interest for our first research question. The effect of *Sample Delay* is the subject of our second research question. We consider the *Defecation Day* as a potential confounder, as the deer's stress level exhibits seasonal patterns [@Vilela2020]. The *Number of Other Hunts* reflects the intensity of recent hunting activity. In time periods of frequent hunting events, a deer might have a generally higher stress level, or adaptation might occur which dampens the stress response towards new hunting events.

We propose a generalized additive mixed model (GAMM) to account for potentially non-linear effects of covariates and potential individual differences between the observed deer. We use the R package `mgcv` [@mgcv] to fit the models. The covariates *Time Difference*, *Distance*, *Sample Delay*, and *Defecation Day* are encoded by penalized cubic regression splines. We do not use a cyclical spline for *Defecation Day*, because faecal samples were not collected during winter and therefore *Defecation Day* is not a cyclical variable. The *Number of Other Hunts* is adopted as a linear effect. We further introduce a random intercept for each deer.
...
We propose a xgboost model using the R package `xgboost` [@ChenXgboost], choosing *Distance* and *Time Difference* as covariates. To find the best fitting model for each dataset (see @tbl-datasets), we tune hyperparamters of the proposed xgboost model. We choose to tune the following paramters, using randomized grid search [@Bergstra2012]:
...
In models estimated with the REML method, the smooth effects are penalized to almost linear, as shown in @fig-gamm-reml. While the fitted curves have a similar tendency across the datasets, the standard error in estimation is generally large. Given such high uncertainty, no interpretation of the estimates is reliable. The GCV method yields more wiggly smooth effect estimates, which are visualized in @fig-gamm-gcv, but high uncertainty persists. For the other covariates, we also observe high uncertainty and therefore avoid making any unreliable statement.
...
The XGBoost model outperforms GAMM on the "highest score" and "nearest" dataset in terms of model fit measured by root mean squared error (RMSE), but performs worse on the "closest in time" dataset than the GAMM estimated with GCV (see @tbl-rmse). The tuned hyperparameters are reported in @tbl-xgboost_hp.
...
We believe that more precise information about hunting events (e.g., tracking timespans rather than single moments and enforcing complete timestamps), higher temporal resolution in movement tracking, and accounting for more confounders such as weather could reduce uncertainty and yield clearer results.
...
![Nearest](Figures/Models/nearest_GCV.Cp_diagnostic_custom.png)

![Highest score](Figures/Models/Highest score_GCV.Cp_diagnostic_custom.png)
