...
The XGBoost model outperforms both GAMM models on the "Highest score" and "Nearest" dataset in predictive power but performs worse on the "Closest in time" dataset. After reviewing the hyperparameters and comparisons between models trained on the actual data and the data with randomized labels we concluded that the XGBoost models likely did not learn noise and therefore are not overfitting.

<!-- ![Predictions of XGBoost models on the three datasets. The points are all observations within each dataset.](Figures/Models/xgboost_combined.png){#fig-xgboost_predictions fig-env="figure*"} -->
...
One would expect a decrease in the FCM level closer to the zero point, however we did not observe this for models trained on the first two datasets. The highest score dataset shows a slight increase close to zero however the same model also predicts a high "plateau" at 33+ hours & 11.5 - 16 km. The XGBoost models show a relatively weak and very unclear effect.
...
#| tbl-cap: "Hyperparameters of XGBoost models on the three datasets."
...
![3D Visualization of the XGBoost models.](Figures/Models/combined_xgboost_plots.png)

Predictions for the FCM level on the three datasets by the three XGBoost models. The real data are shown as black points.