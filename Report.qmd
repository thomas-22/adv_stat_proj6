...
We were provided with four distinct Datasets.  In the following subchapters we are going to describe the main features of each data set, but the reproduction data (see @sec-feature_selection) and address any anomalies.
...
3.  **Highest score**: Introducing a scoring function to consider both spatial and temporal distance. We designed a score proportional to the inverse square of the distance and a "spike" function based on time difference (see @fig-score). The inverse square component is inspired by physical dynamics[^2], spike function was inspired by the general dynamics of stress responses, incorporating insights from Dr. Ferry and further supported by findings on cortisol fluctuations in brown bears following an ACTH challenge, as described in the study 'Blood cortisol and faecal cortisol metabolite concentrations following an ACTH challenge in unanaesthetized brown bears (Ursus arctos) (https://pmc.ncbi.nlm.nih.gov/articles/PMC11734623/))
(https://en.wikipedia.org/wiki/Pharmacokinetics). The observation with the highest score was selected.

[^2]: such as the inverse square law for sound intensity (http://hyperphysics.phy-astr.gsu.edu/hbase/Acoustic/invsqs.html)
...
For each dataset shown in @tbl-datasets, we consider the five features introduced in @sec-filtering as covariates.
...
<!-- Niko: already have this paragrap in Filtering section -->
...
# GAMM Model Evaluation and Results Summary {#sec-results}

## Overview

In this analysis, we fitted and evaluated GAMM models across three different matching strategies using two smoothing parameter estimation methods: REML and GCV. The evaluation focused on assessing the interpretability of smooth effects and the validity of random effects.
...
Adjusted predictions for the FCM level. Each plot shows the predicted values of the FCM level in dependence of one covariate. The other covariants are held constant at the mean. The grey area shows the approximate confidence band (mean $\pm$ 1.96 standard error).
...
GCV (Generalized Cross-Validation) tends to produce more fluctuating smooth effects due to its weaker control over model complexity. Compared to REML (Restricted Maximum Likelihood), it is more likely to select overly complex models, leading to more wiggly estimated smooth curves.
